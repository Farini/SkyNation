...
void main( void ) {
    ...
    float normPos = (v_path_distance / u_path_length);
...
//    float factory = normPos * timer;
    ...
    // Colors
    float red = max(0.15, 0.25 * secondColor);
    float green = max(0.2, 0.75 * firstColor);
    float blue = max(thirdColor, firstColor); //max(0.5, thirdColor);
